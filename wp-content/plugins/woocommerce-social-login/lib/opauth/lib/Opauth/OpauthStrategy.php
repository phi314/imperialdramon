...
class OpauthStrategy {
	...
	/**
	 * Ensure the state parameter returned in the redirect from the provider matches
	 * what was stored when the login request was initiated. Prevents CSRF for
	 * log-in attempts.
	 *
	 * Note that this *must* be called prior to processing the internal OAuth callback
	 * within a provider.
	 *
	 * @since 1.7.5-1
	 * @return bool
	 */
	protected function is_state_valid() {

		if ( empty( $_GET['state'] ) ) {
			return false;
		}

		$key = 'wcsl_state_' . md5( $_SERVER['REMOTE_ADDR'] . $_SERVER['HTTP_USER_AGENT'] );

		$known_state    = strval( get_transient( $key ) );
		$provided_state = strval( $_GET['state'] );

		delete_transient( $key );

		return hash_equals( $known_state, $provided_state );
	}


	/**
	 * Invalid state callback, used when the state parameter isn't valid
	 *
	 * @since 1.7.5-1
	 */
	protected function invalid_state_callback() {

		$params = array(
			'error'     => array(
				'provider' => $this->strategy['strategy_name'],
				'code'     => 'invalid_state',
				'message'  => 'The provider state parameter does not match.',
				'raw'      => null,
			),
			'timestamp' => date( 'c' ),
		);
