...
abstract class WC_Social_Login_Provider extends WC_Settings_API {
	...
	/** @var string state parameter used for preventing CSRF for providers */
	protected $state;
	...
	public function __construct( $base_auth_path ) {
		...
		if ( $this->use_opauth ) {
			...
			// instantiate opauth to redirect user to provider for authorization
			add_action( 'woocommerce_api_' . $base_auth_path . '/' . $this->id, array( $this, 'redirect_to_provider' ) );

			// instantiate opauth to process the redirect back from the provider, generally this includes the code/state params
			add_action( 'woocommerce_api_' . $base_auth_path . '/' . $this->id . '/' . $this->internal_callback, array( $this, 'redirect_from_provider' ) );
		}
	...
	/**
	 * Instantiate opauth to handle redirecting the user to provider for authorization
	 *
	 * @since 1.7.6
	 */
	public function redirect_to_provider() {
		...
		// set the state parameter as a transient if the provider requires it
		// this is used later by the provider class to ensure the callback hasn't been forged
		if ( $state = $this->get_state() ) {
			set_transient( 'wcsl_state_' . md5( $_SERVER['REMOTE_ADDR'] . $_SERVER['HTTP_USER_AGENT'] ), $state, 5 * MINUTE_IN_SECONDS );
		}

		wc_social_login()->get_opauth_instance()->authenticate();
	}


	/**
	 * Instantiate opauth to handle the redirect back from the provider, which
	 * contains the code/state params, ready for validation and further processing
	 *
	 * @since 1.7.6
	 */
	public function redirect_from_provider() {
	...
	/**
	 * Return the state parameter for this provider
	 *
	 * @since 1.7.6
	 * @return string
	 */
	...
	/**
	 * Generate a random 64 character string for use as the state parameter with
	 * OAuth2 requests. Providers will return this along with the code, which we can
	 * use to verify the authorization request is not spoofed.
	 *
	 * @since 1.7.6
	 * @return string
	 */
	protected function generate_state() {
		...
		// works in all supported versions of WP and PHP
		$state = wp_generate_password( 64, false );

		// but try to use random_bytes() if it exists
